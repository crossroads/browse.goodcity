...
export default Ember.Mixin.create({
  ...
  cancelOrder(order) {
    let cancellationReason = Ember.$(`#cancel-appointment-reason${order.id}`)
      .val()
      .trim();
    if (!cancellationReason.length) {
      this.set("cancellationReasonWarning", true);
      Ember.$(`#cancel-appointment-reason${order.id}`).addClass(
        "cancel-booking-error"
      );
      return false;
    } else {
      Ember.$(`#cancel-appointment-reason${order.id}`).removeClass(
        "cancel-booking-error"
      );
