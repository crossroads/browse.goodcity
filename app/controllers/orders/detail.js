...
export default applicationController.extend({
  ...
  cancelOrder(order) {
    let cancellationReason = Ember.$(`#appointment-cancellation-reason`)
      .val()
      .trim();
    if (!cancellationReason.length) {
      this.set("cancellationReasonWarning", true);
      Ember.$("#appointment-cancellation-reason").addClass(
        "cancel-booking-error"
      );
      return false;
    } else {
      Ember.$("#appointment-cancellation-reason").removeClass(
        "cancel-booking-error"
      );
