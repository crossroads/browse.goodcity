...
module('Acceptance | Account Details Page', {
  beforeEach: function() {
    ...
    $.mockjax({url: "api/v1/gc_org*", responseText: {
       gc_organisations: [gcOrganisations.toJSON({includeId: true})]
    }});
...
test("User redirects to search_organisation page on clicking ", function(assert){
  assert.expect(2);
  visit('/account_details');
  andThen(function(){
    assert.equal(currentURL(), '/account_details');
