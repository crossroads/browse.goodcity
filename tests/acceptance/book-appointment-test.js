...
var App, order, gogo_van, user, user_profile, orderPurpose, organisation, pkg, ordersPackage, mocks, goodcityRequest;
...
test("request purpose page on completely filled should redirect to client information if For client benificiary is selected", function(assert){
  orderPurpose = {id: 1, purpose_id: 2, order_id: order.id, designation_id: 1};
  assert.expect(1);
  visit('/request_purpose');
  andThen(function(){
    click(".for-client");
