...
module("Acceptance | Book appointment/order ", {
  ...
  beforeEach: function() {
    ...
    benificiary = {
      ...
      first_name: "Test",
      id: 12,
      identity_number: "1233",
      identity_type_id: 1,
      last_name: "John",
      phone_number: "+85212312312",
      title: null
    };
    ...
    goodcityRequest = {
      ...
      code_id: null,
      order_id: order.id,
      packageTypes: [{ id: package_type.id }]
    };
    ...
    mocks.push(
      ...
      mockFindAll("booking_type").returns({
        json: { booking_types: _.values(BookingTypes) }
      }),
    ...
    let slots = [
      { id: null, isClosed: false, note: "", quota: 3, remaining: 3, timestamp }
    ];
    mocks.push(
      $.mockjax({
        url: "/api/v1/appointment_slots/calenda*",
        type: "GET",
        status: 200,
        responseText: {
          appointment_calendar_dates: [
            { date: date, slots: slots, isClosed: false }
          ]
        }
      })
    );
...
test("Online order : Filled Up client info page, should redirect to schedule details page on submit", function(assert) {
  orderPurpose2 = {
    ...
    order_id: order.id,
    designation_id: 1,
    booking_type_id: BookingTypes.onlineOrder.id
  };
  assert.expect(2);
  ...
  andThen(function() {
    ...
    andThen(function() {
      let updated_gc_req = {
        id: goodcityRequest.id,
        code_id: package_type.id,
        order_id: order.id,
...
test("Appointment Details Page on incomplete sumission should not redirect to order confirm page", function(assert) {
  ...
  let slots = [
    { id: null, timestamp, quota: 3, isClosed: false, note: "", remaining: 3 }
  ];
  assert.expect(1);
  mocks.push(
    $.mockjax({
    ...
    $.mockjax({
      ...
      responseText: {
        appointment_calendar_dates: [
          { date: date, isClosed: false, slots: slots }
        ]
      }
    }),
    mockFindAll("package_type").returns({
      json: { package_types: [package_type.toJSON({ includeId: true })] }
    })
  );
  visit("/");
  andThen(function() {
