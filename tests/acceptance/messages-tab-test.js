...
var App,
  ...
  orders_package,
  order_transport,
  order_transport2; // , senders, messages, images;
let mocks;
...
module("Acceptance | Messages Tab Test", {
  beforeEach: function() {
    App = startApp();
...
test("clicking on an order opens the summary by default", function(assert) {
  assert.expect(2);
  click(Ember.$(".list-items .order_block")[0]);
  andThen(() => {
    assert.equal(currentURL(), `/orders/${order.id}/booking`);
