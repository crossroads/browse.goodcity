...
module("Acceptance | Item Page", {
  beforeEach: function() {
    ...
    subcategory1 = FactoryGuy.make("package_category", {
      parent_id: parseInt(pkgCategory.id),
      packagr_type_codes: pkgType1.get("code")
    });
    subcategory2 = FactoryGuy.make("package_category", {
      parent_id: parseInt(pkgCategory.id),
      packagr_type_codes: pkgType1.get("code")
    });
    user_profile = {
    ...
    mockFindAll("package_category").returns({
      json: {
        package_categories: [
          pkgCategory.toJSON({ includeId: true }),
          subcategory1.toJSON({ includeId: true }),
          subcategory2.toJSON({ includeId: true })
        ]
      }
    });
    mockFindAll("package_type").returns({
      json: { package_types: [pkgType1.toJSON({ includeId: true })] }
    });
    ...
    mockFindAll("item").returns({
      json: {
        items: [
          item_with_packages.toJSON({ includeId: true }),
          next_item.toJSON({ includeId: true }),
          item.toJSON({ includeId: true }),
          prev_item.toJSON({ includeId: true })
        ]
      }
    });
